...
// BookContainer groups notes by book for rendering
...
// NewBookContainer creates a new BookContainer from a list of notes
func NewBookContainer(title string, notes []*LocalNote) *BookContainer {
	container := &BookContainer{
		Title: title,
		Notes: make([]*LocalNote, 0, len(notes)),
	}

	// Filter notes that have text and sort by time
	for _, note := range notes {
		if note.GetText() != "" {
			container.Notes = append(container.Notes, note)
		}
	}

	// Sort by time
	sortNotesByTime(container.Notes)

	// Extract author from first note
	if len(container.Notes) > 0 {
		container.Author = container.Notes[0].GetAuthor()
	}

	return container
}

// sortNotesByTime sorts notes by their timestamp in ascending order
func sortNotesByTime(notes []*LocalNote) {
	for i := 0; i < len(notes)-1; i++ {
		for j := i + 1; j < len(notes); j++ {
			if notes[i].Time.After(notes[j].Time) {
